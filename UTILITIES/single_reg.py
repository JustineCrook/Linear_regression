...
def run_linear_regression_with_uplims(i, nx=None, verbose=False, ignore_uplims=False):
    
    xdet, ydet, xdet_err, ydet_err, xuplim, yuplim, xuplim_err, yuplim_err, true_vals = gen_synthetic_data(seed=i, nx=nx, verbose=False)
    ...
    mcmc = MCMC(kernel, num_warmup=5000, num_samples=1000,progress_bar=verbose)
...
def run_linear_regression_marginalised_xt(i, nx=None,  verbose=False, ignore_uplims=False):
...
def linmix_comparison(i, nx=100, verbose =False, ignore_uplims=False):
    """
    Compare the results of the linmix package with the results of this method.
    """
    
    xobs, xerr, yobs, yerr, delta, true_vals = gen_synthetic_data(seed=i, nx=nx, return_alt=True, verbose=False)
    if ignore_uplims:
        mask = delta==1
        xobs = xobs[mask]
        yobs = yobs[mask]
        xerr = xerr[mask]
        yerr = yerr[mask]
        delta = np.ones_like(xobs).astype(bool)
...
def runner_with_uplims(nrepeats, nx=100, parallel=False, start_i=0,  verbose=False, change_line=False, type_test="marg", ignore_uplims=False):
    ...
    if parallel:
        parallel_results = Parallel(n_jobs=-1)(
            delayed(func)(start_i + i, nx, verbose=False,  ignore_uplims=ignore_uplims )
        ...
        for i in range(nrepeats):
            ...
            normalised_results, samples_arr = func(start_i + i, nx, verbose=verbose,  ignore_uplims=ignore_uplims )
    ...
    if change_line: 
        truth = []
        for i in range(nrepeats):
            _, _, _, _, _, _, _, _, true_vals = gen_synthetic_data(seed=start_i+i, nx=nx, verbose=False, change_line=True)
    ...
    # Corner plot
    if change_line == False: # true values are the same for all runs
